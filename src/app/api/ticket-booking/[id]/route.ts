...
import { TicketBookingStatusUpdateData } from '@/types/ticketBooking'; // Import type đã được cập nhật
...
export async function PUT(
    ...
    try {
        ...
        const newStatus = body.status; // newStatus sẽ có kiểu 'pending' | 'confirmed' | 'cancelled' | 'delivered'
        console.log(`   Received new status: ${newStatus}`);

        // 5. Validate status mới
        // SỬA Ở ĐÂY: Thêm 'delivered' vào mảng kiểm tra
        const allowedStatuses: Array<TicketBookingStatusUpdateData['status']> = ['pending', 'confirmed', 'cancelled', 'delivered'];
        if (!newStatus || !allowedStatuses.includes(newStatus)) {
            console.error(`   Validation Error: Invalid status value received: ${newStatus}. Allowed: ${allowedStatuses.join(', ')}`);
        ...
        const updatedBooking = await TicketBooking.findByIdAndUpdate(
            id,
            { status: newStatus },
            { new: true, runValidators: true } // Trả về document mới, chạy validators của Mongoose (bao gồm enum)
        );
        ...
        if (error.name === 'ValidationError') { // Lỗi từ Mongoose schema validation
            console.error("   Mongoose Validation Error Details:", error.errors);
            return NextResponse.json({ error: `Lỗi validation từ Mongoose: ${error.message}` }, { status: 400 });
        }
